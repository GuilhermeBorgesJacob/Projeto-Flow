...
class scalarField{
    ...
    public:
        ...
        double Ddx_forward(int i,int j) {
            //Derivada total em relação à x
            return (Ddxi_forward(i,j)*y.Ddeta[i][j] - Ddeta_forward(i,j)*y.Ddxi[i][j])/J[i][j];
        }
        double Ddy_forward(int i,int j) {
            //Derivada total em relação à y
            return (Ddeta_forward(i,j)*x.Ddxi[i][j] - Ddxi_forward(i,j)*x.Ddeta[i][j])/J[i][j];
        }
        double Ddx_rearward(int i,int j) {
            //Derivada total em relação à x
            return (Ddxi_rearward(i,j)*y.Ddeta[i][j] - Ddeta_rearward(i,j)*y.Ddxi[i][j])/J[i][j];
        }
        double Ddy_rearward(int i,int j) {
            //Derivada total em relação à y
            return (Ddeta_rearward(i,j)*x.Ddxi[i][j] - Ddxi_rearward(i,j)*x.Ddeta[i][j])/J[i][j];
        }
        ...
        double Ddxi_forward(int i,int j) {
            if (j == columns-1) {
                return (F[i][1] - F[i][j]);
            }
            return (F[i][j+1] - F[i][j]);
        }
        double Ddxi_rearward(int i,int j) {
            if (j == 0) {
                return (F[i][j] - F[i][columns-2]);
            }
        ...
        double Ddeta_forward(int i,int j) {
            if (i==0) {
                return (F[i][j] - F[i+1][j]);
            }
            return (F[i-1][j] - F[i][j]);
        }
        double Ddeta_rearward(int i,int j) {
            if (i==lines-1) {
                return (F[i-1][j] - F[i][j]);
            }
            return (F[i][j] - F[i+1][j]);
...
class flowSim{
    ...
    void calculateUEF() {
        ...
        for (int i = 0; i < u.x.lines; i++) {
            for (int j = 0; j < u.x.columns; j++) {
                ...
                E4.F[i][j] = (U4.F[i][j] + p.F[i][j] - TAUxx.F[i][j])*u.F[i][j] + qx.F[i][j] - v.F[i][j]*TAUxy.F[i][j];
                ...
                F4.F[i][j] = (U4.F[i][j] + p.F[i][j] - TAUyy.F[i][j])*v.F[i][j] + qy.F[i][j] - u.F[i][j]*TAUxy.F[i][j];
            }
        }
    }
    void calculateU(){
    ...
    void calculateE(std::string step){
        if (step == "predictor") {
            #pragma omp parallel for
            for (int i = 0; i < u.x.lines; i++) {
                for (int j = 0; j < u.x.columns; j++) {
                    TAUxx.F[i][j] = (2.0/3.0)*mu.F[i][j]*(2.0*u.Ddx_rearward(i,j) - v.Ddy(i,j));
                    TAUxy.F[i][j] = mu.F[i][j]*(u.Ddy(i,j) + v.Ddx_rearward(i,j));
                    TAUyy.F[i][j] = (2.0/3.0)*mu.F[i][j]*(2.0*v.Ddy(i,j) - u.Ddx_rearward(i,j));
                    qx.F[i][j] = - k.F[i][j]*T.Ddx_rearward(i,j);
                }
            }
        }
        if (step == "corrector") {
            ...
            for (int i = 0; i < u.x.lines; i++) {
                for (int j = 0; j < u.x.columns; j++) {
                    TAUxx.F[i][j] = (2.0/3.0)*mu.F[i][j]*(2.0*u.Ddx_forward(i,j) - v.Ddy(i,j));
                    TAUxy.F[i][j] = mu.F[i][j]*(u.Ddy(i,j) + v.Ddx_forward(i,j));
                    TAUyy.F[i][j] = (2.0/3.0)*mu.F[i][j]*(2.0*v.Ddy(i,j) - u.Ddx_forward(i,j));
                    qx.F[i][j] = - k.F[i][j]*T.Ddx_forward(i,j);
                }
            }
        }
        #pragma omp parallel for
    ...
    void calculateF(std::string step){
        if (step == "predictor") {
            #pragma omp parallel for
            for (int i = 0; i < u.x.lines; i++) {
                for (int j = 0; j < u.x.columns; j++) {
                    TAUxx.F[i][j] = (2.0/3.0)*mu.F[i][j]*(2.0*u.Ddx(i,j) - v.Ddy_rearward(i,j));
                    TAUxy.F[i][j] = mu.F[i][j]*(u.Ddy_rearward(i,j) + v.Ddx(i,j));
                    TAUyy.F[i][j] = (2.0/3.0)*mu.F[i][j]*(2.0*v.Ddy_rearward(i,j) - u.Ddx(i,j));
                    qy.F[i][j] = - k.F[i][j]*T.Ddy_rearward(i,j);
                }
            }
        }
        if (step == "corrector") {
            ...
            for (int i = 0; i < u.x.lines; i++) {
                for (int j = 0; j < u.x.columns; j++) {
                    TAUxx.F[i][j] = (2.0/3.0)*mu.F[i][j]*(2.0*u.Ddx(i,j) - v.Ddy_forward(i,j));
                    TAUxy.F[i][j] = mu.F[i][j]*(u.Ddy_forward(i,j) + v.Ddx(i,j));
                    TAUyy.F[i][j] = (2.0/3.0)*mu.F[i][j]*(2.0*v.Ddy_forward(i,j) - u.Ddx(i,j));
                    qy.F[i][j] = - k.F[i][j]*T.Ddy_forward(i,j);
                }
            }
        }
        #pragma omp parallel for
    ...
    public:
        ...
        void apply(flowSim &flow) {
            ...
            for (int j = 0; j < flow.u.columns; j++) {
                ...
                flow.p.F[i][j] = (- flow.p.F[i-2][j] + 4.0*flow.p.F[i-1][j])/3.0;
                flow.rho.F[i][j] = flow.p.F[i][j]/(flow.R*T_wall);
            }
            #pragma omp parallel for
            for (int i = 0; i < flow.u.lines; i++) {
                for (int j = 0; j < flow.u.columns; j++) {
                    if (flow.rho.F[i][j] < 0 || std::isnan(flow.rho.F[i][j]) || std::isinf(flow.rho.F[i][j])) {
                        flow.rho.F[i][j] = rho_infinity;
                    }
                    if (flow.e.F[i][j] < 0 || std::isnan(flow.e.F[i][j]) || std::isinf(flow.e.F[i][j])) {
                        flow.e.F[i][j] = flow.Cv*p_infinity/((flow.R)*rho_infinity);
                    }
                    if ( flow.p.F[i][j] < 0 || std::isnan(flow.p.F[i][j]) || std::isinf(flow.p.F[i][j])) {
                        flow.p.F[i][j] = p_infinity;
                    }
                    if (flow.T.F[i][j] < 0 || std::isnan(flow.T.F[i][j]) || std::isinf(flow.T.F[i][j])) {
                        flow.T.F[i][j] = p_infinity/(flow.R*rho_infinity);
                    }
                    if (std::isnan(flow.u.F[i][j]) || std::isinf(flow.u.F[i][j])) {
                        flow.u.F[i][j] = u_infinity;
                    }
                    if (std::isnan(flow.v.F[i][j]) || std::isinf(flow.v.F[i][j])) {
                        flow.v.F[i][j] = v_infinity;
                    }
                }
            }
...
bool deuInf(flowSim &flow);
void ondeDeuInf(flowSim &flow);

int main() {
    std::string gridFileName = "cilindro.grid";
    std::string outputFileName = "escoamento.vtk";
    std::string outputFileName2 = "escoamentoFinal.vtk";
    std::string outputFileName3 = "escoamento_mil_iter_antes.vtk";
    ...
    for (int iteration = 1; iteration <= MAXiterations; iteration++) {
        ...
        if (deuNan(predictedFlow)) {
            std::cout << "Deu nan predicted flow na iteracao " << iteration << '\n';
            ondeDeuNan(predictedFlow);
            writeVTKfile(initialFlow, "antesDedarRuim.vtk");
            writeVTKfile(predictedFlow, outputFileName);
            writeVTKfile(predictedFlow, outputFileName3);
            break;
        }
        if (deuInf(predictedFlow)) {
            std::cout << "Deu inf predicted flow na iteracao " << iteration << '\n';
            ondeDeuInf(predictedFlow);
            writeVTKfile(initialFlow, "antesDedarRuim.vtk");
            writeVTKfile(predictedFlow, outputFileName);
            writeVTKfile(predictedFlow, outputFileName3);
            break;
        ...
        if (deuNan(correctedFlow)) {
            std::cout << "Deu nan corrected flow na iteracao " << iteration << '\n';
            ondeDeuNan(correctedFlow);
            writeVTKfile(predictedFlow, "antesDedarRuim.vtk");
            writeVTKfile(correctedFlow, outputFileName);
            writeVTKfile(correctedFlow, outputFileName3);
            break;
        }
        if (deuInf(correctedFlow)) {
            std::cout << "Deu inf corrected flow na iteracao " << iteration << '\n';
            ondeDeuInf(correctedFlow);
            writeVTKfile(predictedFlow, "antesDedarRuim.vtk");
            writeVTKfile(correctedFlow, outputFileName);
            writeVTKfile(correctedFlow, outputFileName3);
            break;
        ...
        if (iteration%1000 == 0) {
            ...
            std::cout << duration.count() << " segundos ate a iteracao "<<iteration<< '\n';
            start = std::chrono::high_resolution_clock::now();
        }
    }
    writeVTKfile(predictedFlow, outputFileName2);
    return 0;
}
...
void macCormack(flowSim &flow, flowSim &predictedFlow, flowSim &correctedFlow, boundaryConditionsClass &boundaryConditions) {
    ...
    flow.dynamicViscosity();
    flow.thermalConductivity();
    flow.calculateU();
    flow.calculateE("predictor");
    flow.calculateF("predictor");
    #pragma omp parallel for
    for (int i = 0; i < flow.u.lines; i++) {
        for (int j = 0; j < flow.u.columns; j++) {
            predictedFlow.U1.F[i][j] = flow.U1.F[i][j] - (flow.E1.Ddx_forward(i,j) + flow.F1.Ddy_forward(i,j))*dt;
            predictedFlow.U2.F[i][j] = flow.U2.F[i][j] - (flow.E2.Ddx_forward(i,j) + flow.F2.Ddy_forward(i,j))*dt;
            predictedFlow.U3.F[i][j] = flow.U3.F[i][j] - (flow.E3.Ddx_forward(i,j) + flow.F3.Ddy_forward(i,j))*dt;
            predictedFlow.U4.F[i][j] = flow.U4.F[i][j] - (flow.E4.Ddx_forward(i,j) + flow.F4.Ddy_forward(i,j))*dt;
        }
    }
    ...
    predictedFlow.dynamicViscosity();
    predictedFlow.thermalConductivity();
    predictedFlow.calculateU();
    predictedFlow.calculateE("corrector");
    predictedFlow.calculateF("corrector");
    #pragma omp parallel for
    for (int i = 0; i < flow.u.lines; i++) {
        for (int j = 0; j < flow.u.columns; j++) {
            correctedFlow.U1.F[i][j] = 0.5*(predictedFlow.U1.F[i][j] + flow.U1.F[i][j] - (predictedFlow.E1.Ddx_rearward(i,j) + predictedFlow.F1.Ddy_rearward(i,j))*dt);
            correctedFlow.U2.F[i][j] = 0.5*(predictedFlow.U2.F[i][j] + flow.U2.F[i][j] - (predictedFlow.E2.Ddx_rearward(i,j) + predictedFlow.F2.Ddy_rearward(i,j))*dt);
            correctedFlow.U3.F[i][j] = 0.5*(predictedFlow.U3.F[i][j] + flow.U3.F[i][j] - (predictedFlow.E3.Ddx_rearward(i,j) + predictedFlow.F3.Ddy_rearward(i,j))*dt);
            correctedFlow.U4.F[i][j] = 0.5*(predictedFlow.U4.F[i][j] + flow.U4.F[i][j] - (predictedFlow.E4.Ddx_rearward(i,j) + predictedFlow.F4.Ddy_rearward(i,j))*dt);
        }
    }
...
void writeVTKfile(flowSim &flow, std::string fileName) {
    ...
    tecplot.open(fileName,std::fstream::out|std::fstream::trunc);
        if(tecplot.is_open()){
            ...
            for (int i = 0; i < flow.u.lines; i++){
                for (int j = 0; j < flow.u.columns; j++){
                    tecplot << static_cast<float>(flow.p.F[i][j]) << "\n";
                }
            }
            ...
            for (int i = 0; i < flow.u.lines; i++){
                for (int j = 0; j < flow.u.columns; j++){
                    tecplot << static_cast<float>(flow.rho.F[i][j]) << "\n";
                }
            }
            ...
            for (int i = 0; i < flow.u.lines; i++){
                for (int j = 0; j < flow.u.columns; j++){
                    tecplot << static_cast<float>(flow.T.F[i][j]) << "\n";
                }
            }
            ...
            for (int i = 0; i < flow.u.lines; i++){
                for (int j = 0; j < flow.u.columns; j++){
                    tecplot << static_cast<float>(flow.e.F[i][j]) << "\n";
                }
            }
            // Velocidade (u, v)
            ...
            for (int i = 0; i < flow.u.lines; i++){
                for (int j = 0; j < flow.u.columns; j++){
                    tecplot << static_cast<float>(flow.u.F[i][j]) << " " << static_cast<float>(flow.v.F[i][j]) << " 0.0\n";
                }
            }
...
bool deuInf(flowSim &flow) {
    for (int i = 0; i < flow.u.lines; i++) {
        for (int j = 0; j < flow.u.columns; j++) {
            if (std::isinf(flow.rho.F[i][j])) { std::cout << "inf: rho i,j=" << i << "," << j <<'\n'; return true;}
            else if (std::isinf(flow.u.F[i][j])) { std::cout << "inf: u i,j=" << i << "," << j <<'\n'; return true;}
            else if (std::isinf(flow.v.F[i][j])) { std::cout << "inf: v i,j=" << i << "," << j <<'\n'; return true;}
            else if (std::isinf(flow.p.F[i][j])) { std::cout << "inf: p i,j=" << i << "," << j <<'\n'; return true;}
            else if (std::isinf(flow.e.F[i][j])) { std::cout << "inf: e i,j=" << i << "," << j <<'\n'; return true;}
            else if (std::isinf(flow.T.F[i][j])) { std::cout << "inf: T i,j=" << i << "," << j <<'\n'; return true;}
            else if (std::isinf(flow.mu.F[i][j])) { std::cout << "inf: mu i,j=" << i << "," << j <<'\n'; return true;}
            else if (std::isinf(flow.k.F[i][j])) { std::cout << "inf: k i,j=" << i << "," << j <<'\n'; return true;}
            else if (std::isinf(flow.TAUxx.F[i][j])) { std::cout << "inf: TAUxx i,j=" << i << "," << j <<'\n'; return true;}
            else if (std::isinf(flow.TAUxy.F[i][j])) { std::cout << "inf: TAUxy i,j=" << i << "," << j <<'\n'; return true;}
            else if (std::isinf(flow.TAUyy.F[i][j])) { std::cout << "inf: TAUyy i,j=" << i << "," << j <<'\n'; return true;}
            else if (std::isinf(flow.qx.F[i][j])) { std::cout << "inf: qx i,j=" << i << "," << j <<'\n'; return true;}
            else if (std::isinf(flow.qy.F[i][j])) { std::cout << "inf: qy i,j=" << i << "," << j <<'\n'; return true;}
        }
    }
    return false;
}
...
void ondeDeuInf(flowSim &flow) {
    ...
    for (int i = 0; i < flow.u.lines; i++) {
        for (int j = 0; j < flow.u.columns; j++) {
            if (std::isinf(flow.u.F[i][j])) {std::cout << "Inf: u i,j=" << i << "," << j <<'\n';}}}
    for (int i = 0; i < flow.u.lines; i++) {
        for (int j = 0; j < flow.u.columns; j++) {
            if (std::isinf(flow.v.F[i][j])) {std::cout << "Inf: v i,j=" << i << "," << j <<'\n';}}}
    for (int i = 0; i < flow.u.lines; i++) {
        for (int j = 0; j < flow.u.columns; j++) {
            if (std::isinf(flow.p.F[i][j])) {std::cout << "Inf: p i,j=" << i << "," << j <<'\n';}}}
    for (int i = 0; i < flow.u.lines; i++) {
        for (int j = 0; j < flow.u.columns; j++) {
            if (std::isinf(flow.e.F[i][j])) {std::cout << "Inf: e i,j=" << i << "," << j <<'\n';}}}
    for (int i = 0; i < flow.u.lines; i++) {
        for (int j = 0; j < flow.u.columns; j++) {
            if (std::isinf(flow.T.F[i][j])) {std::cout << "Inf: T i,j=" << i << "," << j <<'\n';}}}
    for (int i = 0; i < flow.u.lines; i++) {
        for (int j = 0; j < flow.u.columns; j++) {
            if (std::isinf(flow.mu.F[i][j])) {std::cout << "Inf: mu i,j=" << i << "," << j <<'\n';}}}
    for (int i = 0; i < flow.u.lines; i++) {
        for (int j = 0; j < flow.u.columns; j++) {
            if (std::isinf(flow.k.F[i][j])) {std::cout << "Inf: k i,j=" << i << "," << j <<'\n';}}}
    for (int i = 0; i < flow.u.lines; i++) {
        for (int j = 0; j < flow.u.columns; j++) {
            if (std::isinf(flow.TAUxx.F[i][j])) {std::cout << "Inf: TAUxx i,j=" << i << "," << j <<'\n';}}}
    for (int i = 0; i < flow.u.lines; i++) {
        for (int j = 0; j < flow.u.columns; j++) {
            if (std::isinf(flow.TAUxy.F[i][j])) {std::cout << "Inf: TAUxy i,j=" << i << "," << j <<'\n';}}}
    for (int i = 0; i < flow.u.lines; i++) {
        for (int j = 0; j < flow.u.columns; j++) {
            if (std::isinf(flow.TAUyy.F[i][j])) {std::cout << "Inf: TAUyy i,j=" << i << "," << j <<'\n';}}}
    for (int i = 0; i < flow.u.lines; i++) {
        for (int j = 0; j < flow.u.columns; j++) {
            if (std::isinf(flow.qx.F[i][j])) {std::cout << "Inf: qx i,j=" << i << "," << j <<'\n';}}}
    for (int i = 0; i < flow.u.lines; i++) {
        for (int j = 0; j < flow.u.columns; j++) {
            if (std::isinf(flow.qy.F[i][j])) {std::cout << "Inf: qy i,j=" << i << "," << j <<'\n';}}}
}